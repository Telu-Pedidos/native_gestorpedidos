...
export function CategoryDelete({ id, name, fetchData }: CategoryDeleteProps) {
  ...
  async function handleDelete() {
    startTransition(async () => {
      try {
        ...
        console.error(error);
        toast({
          title: "ocorreu um erro ao tentar excluir a categoria",
          variant: "error",
        });
      }
    });
          ...
          <AlertDialogBody className="mb-4 mt-3">
            <View className="mb-4 flex flex-row gap-2 text-destructive">
              ...
              <Text className="flex flex-col gap-1 text-base font-medium text-destructive">
                Atenção! Você está prestes a excluir a categoria{" "}
